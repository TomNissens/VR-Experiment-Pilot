...
class Parser:
    def __init__(self):
        ...
        # but analyze them only if they are .asc files
        self.session = {}
        nroffiles = 0
        for file in os.listdir("./data"):
            ...
            if ext == ".csv" and root[:9] == "data_hand":
                ...
                self.participants[self.pp_code] = \
                    {'pp_nr': root[10:12], 'pp_id': root[12:14]}
                print(self.pp_code)
        ...
        if self.count_resample_error > 0:
            raise RuntimeError('total nr of trials failed to resample' +
                               ' (outside of current exclussion criteria): '
                               + str(self.count_resample_error))
    ...
    def parse(self, file):
        ...
        for line in open(os.path.join("data", file)):
            ...
            # and convert these into floats and integers if possible
            str_word = line.split()
            word = []
            ...
            if word[1] == 'start_trial':
                trialid = int(word[2])
                trial[trialid] = {'TRIALNR': trialid,
                                  'FILE': file,
                                  'TRIAL_START': int(word[0])}
                ...
                for idx in range(1, len(trial[trialid]['hand_samples'])):
                    velocities.append(self.velocity_calc(trial[trialid]['hand_samples'][idx-1],
                                                         trial[trialid]['hand_samples'][idx]))
                ...
                for idx in range(1, len(trial[trialid]['hand_samples_interp_filt'])):
                    ...
                    if not trial[trialid]['on_shape_t'] == -1\
                        and trial[trialid]['on_shape_velo'] == -1\
                            and trial[trialid]['hand_samples_interp'][idx, 0] > trial[trialid]['on_shape_t']:
                    ...
                    if velocities[-1] > 0.2 and trial[trialid]['move_start_samplenr'] == -1:
                        ...
                        trial[trialid]['reach_latency'] = trial[trialid][
                            'hand_samples_interp_filt'][
                                len(velocities), 0] - trial[trialid]['start_task_t']
                    elif velocities[-1] < 0.5\
                        and not trial[trialid]['on_shape_samplenr'] == -1\
                            and trial[trialid]['move_end_samplenr'] == -1:
                ...
                if not self.data_log.finger_on_shape[trialid] == -1 and trial[trialid]['exp_type'] == 'experiment':
                    ...
                    trial[trialid]['reach_interuption'] = self.detectReachInterupt(
                        np.copy(trial[trialid]['velocities_filtered'][trial[trialid]['move_start_samplenr']:
                                                                      trial[trialid]['on_shape_samplenr']]))

                    if trial[trialid]['reach_interuption'] == 0:
                        # cut trajectory from samples
                        trial[trialid]['traj'] = np.copy(trial[trialid]['hand_samples_interp_filt']
                                                         [trial[trialid]['move_start_samplenr']:
                                                          trial[trialid]['on_shape_samplenr']])
                        # resample traj
                        trial[trialid]['traj_resample'], trial[trialid]['resample_success']\
                            = self.resamplenorm(np.copy(trial[trialid]['traj']), self.resample_nr)
                ...
                trial[trialid]['hand_samples'] = np.append(trial[trialid]['hand_samples'],
    ...
    def pp_process(self, trial):
        ...
        for tar_pos in range(4):
            ...
            trial_count = 0
            for t_nr in self.data_log.index[(self.data_log.tar_pos == tar_pos)
                                            & (self.data_log.dis_condition == 'same')
                                            & (self.data_log.finger_on_target == 1)]:
            ...
            for dis_pos in dis_range:
                self.participants[self.pp_code]['distractor_traj'+str(tar_pos)+str(dis_pos)]\
                    = np.zeros([self.resample_nr, 3])
                trial_count = 0
                for t_nr in self.data_log.index[(self.data_log.tar_pos == tar_pos)
                                                & (self.data_log.dis_pos_rel == dis_pos)
                                                & (self.data_log.dis_condition == 'different')
                                                & (self.data_log.finger_on_target == 1)]:
                    if trial[t_nr]['resample_success'] == 1:
                        self.participants[self.pp_code]['distractor_traj'+str(tar_pos)+str(dis_pos)]\
                            += trial[t_nr]['traj_resample']
                        trial_count += 1
                ...
                if dis_pos < 0:
                    self.participants[self.pp_code]['overall_traj']\
                        -= (self.participants[self.pp_code]['distractor_traj'+str(tar_pos)+str(dis_pos)]
                            - self.participants[self.pp_code]['baseline_traj'+str(tar_pos)])
                else:
                    self.participants[self.pp_code]['overall_traj']\
                        += (self.participants[self.pp_code]['distractor_traj'+str(tar_pos)+str(dis_pos)]
                            - self.participants[self.pp_code]['baseline_traj'+str(tar_pos)])
    ...
    def pp_figs(self, trial):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            for tar_pos in range(4):
                ...
                for tr in self.data_log.index[(self.data_log.tar_pos == tar_pos)
                                              & (self.data_log.finger_on_target == 1)]:
    ...
    def plotAvgTrajConditionsPP(self, version='baseline', mode='separate'):
        ...
        color_list = [[1, 0.25, 0.25], [0.9, 0.25, 0.25], [0.8, 0.25, 0.25],
                      [0.25, 0.25, 0.8], [0.25, 0.25, 0.9], [0.25, 0.25, 1]]
        line_stylist = ['-.', ':', '-']
